...
const PLOT_OR_DATA_TYPES: ReadonlySet<string> = new Set([
  'physicsDataPlot',
  'mathFunctionPlot',
  'functionPlot',
  'coordinateSystem2D',
...
export const boundsForObject = (element: Record<string, unknown>): SceneBounds | null => {
  ...
  const isPlotOrData = PLOT_OR_DATA_TYPES.has(type);

  if (!isPlotOrData && Array.isArray(element.points) && (type === 'pen' || type === 'line' || !type)) {
  ...
  if (Number.isFinite(element.x) && Number.isFinite(element.y)) {
    ...
    if (rotation === 0) {
      ...
      for (const { px, py } of corners) {
        const dx = px - cx;
        const dy = py - cy;
        add(cx + dx * cos - dy * sin, cy + dx * sin + dy * cos);
      }
    }
  }
...
const tilesFor = (
  ...
  const sceneWidth = bounds.x2 - bounds.x1;
  const sceneHeight = bounds.y2 - bounds.y1;
  if (sceneWidth <= PAGED_TILE.w && sceneHeight <= PAGED_TILE.h) {
    return [{
      x1: bounds.x1,
      y1: bounds.y1,
      x2: bounds.x1 + Math.max(sceneWidth, PAGED_TILE.w),
      y2: bounds.y1 + Math.max(sceneHeight, PAGED_TILE.h)
    }];
  }

  const originX = bounds.x1;
  const originY = bounds.y1;
  const tileMap = new Map<string, { tx: number; ty: number; bounds: SceneBounds }>();

  for (const element of scene) {
    const box = boundsForObject(element);
    if (!box) continue;

    const txMin = Math.floor((box.x1 - originX) / PAGED_TILE.w);
    const txMax = Math.floor((box.x2 - 0.001 - originX) / PAGED_TILE.w);
    const tyMin = Math.floor((box.y1 - originY) / PAGED_TILE.h);
    const tyMax = Math.floor((box.y2 - 0.001 - originY) / PAGED_TILE.h);

    for (let ty = tyMin; ty <= tyMax; ty += 1) {
      for (let tx = txMin; tx <= txMax; tx += 1) {
        const key = `${tx},${ty}`;
        if (!tileMap.has(key)) {
          if (tileMap.size >= maxPages) {
            throw new ArtifactCodecError(
              'resource.pdfTooManyPages',
              polishArtifactMessage('resource.pdfTooManyPages', { max: maxPages })
            );
          }
          tileMap.set(key, {
            tx,
            ty,
            bounds: {
              x1: originX + tx * PAGED_TILE.w,
              y1: originY + ty * PAGED_TILE.h,
              x2: originX + (tx + 1) * PAGED_TILE.w,
              y2: originY + (ty + 1) * PAGED_TILE.h
            }
          });
        }
      }
  ...
  if (!tileMap.size) return [bounds];

  const sorted = Array.from(tileMap.values()).sort((a, b) => {
    if (a.ty !== b.ty) return a.ty - b.ty;
    return a.tx - b.tx;
  });

  return sorted.map((t) => t.bounds);
};

...
export const createOperationalSignals = (
  ...
  const measure: OperationalSignals['measure'] = (sample) => {
    try {
      ...
      if (asNumber != null) {
        ...
        if (!aggregate.samples) aggregate.samples = [];
        if (aggregate.samples.length >= 500) aggregate.samples.shift();
        aggregate.samples.push(asNumber);
      }
  ...
  const computePercentile = (samples: number[] | undefined, p: number, fallback: number): number => {
    ...
    const item = sorted[Math.max(0, Math.min(sorted.length - 1, index))];

...
const MAX_WINDOW_MAP_ENTRIES = 20_000;
...
export const pruneExpiredWindows = (map: Map<string, WindowCounter>, now: number): void => {
...
const bumpWindow = (
  ...
  // Actively evict expired keys to bound memory under client churn
  pruneExpiredWindows(map, now);

  // If still at capacity under extreme concurrent churn, evict oldest entries
  if (map.size >= MAX_WINDOW_MAP_ENTRIES && !map.has(key)) {
...
export const createResourceGovernor = (
  ...
  const awarenessWindows = new Map<string, WindowCounter>();
    ...
    try {
      ...
      switch (attempt.kind) {
        ...
        case 'export': {
          ...
          if (attempt.kind === 'export' && pixels > limits.maxExportTilePixels * limits.maxPdfPages) {
...
export const mapGovernorDenialToSocketClose = (
  decision: AdmissionDecision
): SocketCloseDecision => {
  if (decision.decision === 'allow' || decision.decision === 'allowWithBudget') {
    return { code: 1000, reason: 'OK' };
  }
  if (
    decision.reason === 'processConnectionLimit' ||
    decision.reason === 'ipConnectionLimit' ||
    decision.reason === 'boardConnectionLimit'
  ) {
    return { code: 1013, reason: 'Zbyt wiele połączeń. Spróbuj ponownie za chwilę.' };
  }
  if (
    decision.reason === 'payloadTooLarge' ||
    decision.reason === 'updateTooLarge' ||
    decision.reason === 'imageTooLarge'
  ) {
    return { code: 1009, reason: 'Wiadomość przekracza dozwolony rozmiar.' };
  }
  if (decision.reason === 'slowClient') {
    return { code: 1008, reason: 'Połączenie ograniczone — urządzenie nie nadąża.' };
  }
  return { code: 1013, reason: polishResourceMessage(decision.messageKey) };
};

...
export const deriveImageLimitsFromWire = (
  maxWebsocketPayloadBytes: number,
  maxDocumentUpdateBytes: number
): { maxImageDataUrlChars: number; maxEncodedImageBytes: number } => {
  const wireBudget = Math.min(maxWebsocketPayloadBytes - 256, maxDocumentUpdateBytes - 1024);
  const maxImageDataUrlChars = Math.max(1024, wireBudget);
  const base64Chars = Math.max(0, maxImageDataUrlChars - 64);
  const maxEncodedImageBytes = Math.floor((base64Chars * 3) / 4);
  return { maxImageDataUrlChars, maxEncodedImageBytes };
};

const DEFAULT_WS_PAYLOAD_BYTES = 10 * 1024 * 1024;
const DEFAULT_DOC_UPDATE_BYTES = 10 * 1024 * 1024;
const DERIVED_DEFAULT_IMAGE_LIMITS = deriveImageLimitsFromWire(
  DEFAULT_WS_PAYLOAD_BYTES,
  DEFAULT_DOC_UPDATE_BYTES
);
...
export const MEASURED_RESOURCE_LIMITS = {
  ...
  maxWebsocketPayloadBytes: DEFAULT_WS_PAYLOAD_BYTES,
  maxDocumentUpdateBytes: DEFAULT_DOC_UPDATE_BYTES,
  maxHydrationBytes: 64 * 1024 * 1024,
  maxEncodedImageBytes: DERIVED_DEFAULT_IMAGE_LIMITS.maxEncodedImageBytes,
  /** Data-URL character cap (derived from maxWebsocketPayloadBytes / maxDocumentUpdateBytes minus wire overhead). */
  maxImageDataUrlChars: DERIVED_DEFAULT_IMAGE_LIMITS.maxImageDataUrlChars,
  maxDecodedPixelsPerImage: 16_000_000,
  maxPdfBytes: 25 * 1024 * 1024,
  maxPdfPages: 40,
  // Aggregate decoded worksheet budget; 40 ordinary A4 pages at 1.5x fit.
  maxPdfTotalPixels: 96_000_000,
...
export const resourceLimitsFromEnv = (): ResourceLimits => {
  const maxWebsocketPayloadBytes = envInteger(
    'VVE_MAX_WS_PAYLOAD_BYTES',
    MEASURED_RESOURCE_LIMITS.maxWebsocketPayloadBytes
  );
  const maxDocumentUpdateBytes = envInteger(
    'VVE_MAX_DOCUMENT_UPDATE_BYTES',
    MEASURED_RESOURCE_LIMITS.maxDocumentUpdateBytes
  );
  const derived = deriveImageLimitsFromWire(maxWebsocketPayloadBytes, maxDocumentUpdateBytes);
  ...
  return createResourceLimits({
    ...
    maxEncodedImageBytes: envInteger('VVE_MAX_ENCODED_IMAGE_BYTES', derived.maxEncodedImageBytes),
    maxImageDataUrlChars: envInteger('VVE_MAX_IMAGE_DATA_URL_CHARS', derived.maxImageDataUrlChars),
    ...
    maxPdfTotalPixels: envInteger('VVE_MAX_PDF_TOTAL_PIXELS', MEASURED_RESOURCE_LIMITS.maxPdfTotalPixels),

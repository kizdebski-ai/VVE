...
describe('OperationalSignals', () => {
  ...
  it('computes true statistical percentiles for controlled distributions (108-Q1)', () => {
    const signals = createOperationalSignals({ emitJson: false });
    // Controlled distribution: 1 outlier of 100, followed by 19 samples of 1 (total 20 samples)
    signals.measure({ name: 'eventLoop.delayMs', value: 100 });
    for (let i = 0; i < 19; i++) {
      signals.measure({ name: 'eventLoop.delayMs', value: 1 });
    }

    const snapshot = signals.snapshot();
    expect(snapshot.eventLoopDelayMs).not.toBeNull();
    // In a 20-sample set with 19 ones and 1 hundred:
    // p50 is 1 (median)
    expect(snapshot.eventLoopDelayMs?.p50).toBe(1);
    // p95 is 1 (19/20 = 95% of samples are <= 1)
    expect(snapshot.eventLoopDelayMs?.p95).toBe(1);
    // max is 100
    expect(snapshot.eventLoopDelayMs?.max).toBe(100);
    expect(snapshot.eventLoopDelayMs?.min).toBe(1);
  });
});
